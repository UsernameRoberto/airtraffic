...
function addOrUpdatePlane(id, lon, lat, altitude, heading, velocity, timestamp, country) {
  const now = Cesium.JulianDate.fromDate(new Date(timestamp * 1000+7200));
  let plane = planes.get(id);

  // Default model URI
  let modelUri = 'Cesium_Air.glb'; // Default model
...
async function fetchLivePlanes(fetchToken) {
  ...
  try {
    const res = await fetch(`opensky.php?country=${encodeURIComponent(selectedCountry)}`);
...
async function fetchLiveWeatherData(layer) {
  ...
  try {
    ...
    const infoHtml = `
      ...
      👁️ <strong>Visibility:</strong> ${Number(visibility).toLocaleString()}
